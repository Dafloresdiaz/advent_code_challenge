...
def obtain_input():
    ...
    Return: The list of numbers and a list with all the bingo boards
    """
...
def winners_boards():
    ...
    bingo_numbers, boards = obtain_input()
    counts = np.zeros(shape=(len(boards), 5, 5))
    winner_bingo_cards_number = []
    winner_bingo_cards = []
    
    for number in bingo_numbers:
        for i, board in enumerate(boards):
            for y, row in enumerate(board):
                if number in row and i not in winner_bingo_cards_number:
                    index = row.index(number)
                    counts[i][y][index] = 1

        for j, count in enumerate(counts):
            if j not in winner_bingo_cards_number and (any(n for n in count.sum(axis=0) == 5) or any(j for j in count.sum(axis=1) == 5)):
                winner_bingo_cards_number.append(j)
                winner_bingo_cards.append((j, number, np.array(count)))
    
    marked_board = winner_bingo_cards[-1][2]
    board_number = winner_bingo_cards[-1][0]
    winner_number = winner_bingo_cards[-1][1]
    for i, board in enumerate(boards):
        if i == board_number:
            normal_winner_board = board
    for x in range(5):
